...
pats = c("uv001_uvmel_snseq_3p_pre","uv001_uvme_snseq_3p_on","uv002_uvmel_snseq_3p_pre","uv003_uvme_snseq_3p_post")
#maxCounts = c(7500, 6250, 10000, 6000)
#maxFeatures = c(4000, 3500, 4000, 3000)
maxCounts = c(40000, 40000, 40000, 40000)
maxFeatures = c(7500, 7500, 7500, 7500)
...
for (z in 1:length(pats))
{
  ...
  ### Load dataset
  seu.data <- Read10X_h5(paste0('uveal-melanoma/cellbender-newversion/',pat,'/',pat,'_filtered.h5'), use.names = TRUE, unique.features = TRUE)
  ...
  ### Save objects
  saveRDS(seu, file = paste0('uveal-melanoma/cellbender-newversion-same-params/',pat,'/',pat,'_cb.rds'))
  ...
  print(DimPlot(seu, reduction = "umap",label = T,group.by = 'celltype_bped_fine', repel = T) + guides(col = guide_legend(nrow = 30,override.aes = list(size=5))) + theme(legend.text=element_text(size=6)) + ggtitle("celltype_bped_fine"))
  print(DimPlot(seu, reduction = "umap",label = T,group.by = 'celltype_hpca_main', repel = T) + guides(col = guide_legend(nrow = 30,override.aes = list(size=5))) + theme(legend.text=element_text(size=6)) + ggtitle("celltype_hpca_main"))
