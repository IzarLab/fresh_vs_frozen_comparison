...
for (z in 1:length(pat_list_arr))
{
  ...
  for(i in 1:length(pat_list)){
    pat = pat_list[i]
    system(paste0("aws s3 cp ",foldersList[z],"/Seurat/",pat,"_final_thresh/",pat,"_final_thresh_cb.rds /data/",pat,"_final_thresh_cb.rds"))
    patunderscore = str_replace_all(pat,'-','_')
    assign(patunderscore,readRDS(paste0('/data/',pat,'_final_thresh_cb.rds')))
  ...
  for (pat in pat_list) {
    system(paste0("rm /data/",pat,"_final_thresh_cb.rds"))
  }
